...
def evalPerformanceMetrics(trueVals, predVals, prefix='', metrics=['CC', 'R2'], add_mean=True):
    perf = {}
...
class PSIDModel():
    ...
    def set_default_inference(self, default_inference):
        ...
        if default_inference not in allowed:
            raise(Exception(f'Default inference of "{default_inference}" not supported. Must be one of: {allowed}'))
    ...
    def fit(self, Y, Z=None, enable_all_inference=True, time_first=True, **kwargs):        
        ...
        if (iY is None and iZ is None) or self.n1 is None or self.nx is None:
            param_selected, param_sets, param_results, best_param_ind = self._find_hyper_params(Y, Z=Z, **kwargs)
            self.param_selected = param_selected
            self.param_sets = param_sets
            self.param_results = param_results
            self.best_param_ind = best_param_ind
            if 'iY' in param_selected: self.iY = param_selected['iY']
            if 'iZ' in param_selected: self.iZ = param_selected['iZ']
            if 'n1' in param_selected: self.n1 = param_selected['n1']
            if 'nx' in param_selected: self.nx = param_selected['nx']

        iY, iZ = self.get_horizons()
        tic = time.perf_counter()
        model = PSID.PSID(Y=Y, Z=Z, nx=self.nx, n1=self.n1, i=[iY, iZ], time_first=time_first, **kwargs)
        ...
        if enable_all_inference or self.default_inference in ['smoothing']:
            ...
            ZBW = applyFuncIf(subtractIf(ZTF, Zf), np.flipud) if self.bw_on_residual else applyFuncIf(ZTF, np.flipud)
            modelBW = PSID.PSID(Y=YBW, Z=ZBW, nx=self.nx, n1=self.n1, i=[iY, iZ], time_first=True, **kwargs)
            ...
            if isinstance(YTF, (list,tuple)):
                ZfBW = [ZpBW[i] + (fltCzKfBW @ (YBW[i] - YpBW[i]).T).T for i in range(len(YTF))]
            else:
    ...
    def _find_hyper_params(self, Y, Z=None, **kwargs):
        ...
        if 'nx_values' in self.hyper_param_settings and self.hyper_param_settings['nx_values'] is not None:
            nx_values = self.hyper_param_settings['nx_values']
        else:
            nx_values = [self.nx]

        if 'n1_values' in self.hyper_param_settings and self.hyper_param_settings['n1_values'] is not None:
            n1_values = self.hyper_param_settings['n1_values']
        else:
            n1_values = [self.n1] if self.n1 is not None else None

        if 'iY_values' in self.hyper_param_settings and self.hyper_param_settings['iY_values'] is not None:
            iY_values = self.hyper_param_settings['iY_values']
        else:
            iY_min = max(2, int(np.ceil(np.min(np.array(nx_values)) / ny)))
            iY_max = max(iY_min, min(self.hyper_param_settings['max_horizon'], int(n_samples / (ny+1))))
            iY_values = np.unique( np.round(10**np.linspace(np.log10(iY_min), np.log10(iY_max), 3)) ).astype(int)

        if Z is not None:
            nz = Z.shape[-1]
            if 'iZ_values' in self.hyper_param_settings and self.hyper_param_settings['iZ_values'] is not None:
                iZ_values = self.hyper_param_settings['iZ_values']
            else:
                iZ_min = max(2, int(np.ceil(np.min(np.array(n1_values if n1_values is not None else nx_values)) / nz)))
                iZ_max = max(iZ_min, min(self.hyper_param_settings['max_horizon'], int(n_samples / (nz+1))))
                iZ_values = np.unique( np.round(10**np.linspace(np.log10(iZ_min), np.log10(iZ_max), 3)) ).astype(int)
        else:
            iZ_values = [None]
        ...
        for nx_ind, nx in enumerate(nx_values):
            if n1_values is not None:
                ...
                n1_vals = np.unique( np.round(10**np.linspace(0, np.log10(nx), 10)) ).astype(int)
            param_sets = []
            for n1 in n1_vals:
                for iY in iY_values:
                    for iZ in iZ_values:
                        if iY*ny < 2: continue
                        if iY*ny < nx: continue
                        if n1!=0 and iZ*nz < 2: continue
                        if n1!=0 and iZ*nz < n1: continue
        ...
        if len(param_sets) > 1:
            ...
            for param_ind, param in enumerate(param_sets):
                ...
                args = {
                    'logStr': logStr, 'Z': Z, 'Y': Y, 'iCV_folds': iCV_folds, 
                    'nx': nx, 'n1': param['n1'], 'i': param['i'], 
            ...
            if num_processors < min_cpu_for_parallel: # To run serially
                ...
                # To run in parallel
                pool = multiprocessing.Pool(processes=min(len(argsAll), num_processors - 1)) # use all but one of the available processors
            ...
            for param, fold_results in zip(param_sets, results):
                if np.any([(fr['model'] is None) for fr in fold_results]):
                    logger.info(f'Skipping param: {param} [some folds did not have a result]')
                    continue
            ...
            perfStr = ', '.join([f'{metric}={meanPerfVals[metric][best_param_ind]:.3g}' for metric in metrics])
            logger.info(f'nx={nx} => Best parameters were: {param_selected} ({perfStr})')
        else:
            param_results = None
            best_param_ind = 0
