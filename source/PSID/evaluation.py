...
def evalPrediction(trueValue, prediction, measure, missing_marker=None):
    ...
    if measure == 'CC':
        ...
        if np.any(isFlat): # This is the fall-back values for flat true signals in the corrcoef code, but it may not detect flats correctly, so we help it
            perf.setflags(write=1)
            perf[isFlat] = np.nan
    elif measure == 'MSE':
        perf = metrics.mean_squared_error(trueValue, prediction, multioutput='raw_values')
    elif measure == 'RMSE':
        ...
        perf[isFlat] = 0 # This is the fall-back values for flat true signals in the r2_score code, but it may not detect flats correctly, so we help it
    return perf

def evaluateDecoding(ZTest=None, zPredTest=None, YTest=None, yPredTest=None, sId=None, missing_marker=None, YType=None, ZType=None, 
        measures=['CC', 'RMSE', 'R2']):
    """Evaluates prediction of data
    ...
    if zPredTest is not None:
        ...
        zNotBlown = np.all( np.logical_not( np.logical_or(np.isnan(zPredTest), np.isinf(zPredTest)) ), axis=nonTAx)
        ...
        zOk = np.nonzero( np.logical_and(zNotNaN, np.logical_and(zNotBlown, zNotMissing)) )[0]

    if yPredTest is not None:
        nonTAx = tuple(np.arange(1, len(yPredTest.shape)))
        yNotBlown = np.all( np.logical_not( np.logical_or(np.isnan(yPredTest), np.isinf(yPredTest)) ), axis=nonTAx)
        ...
        yOk = np.nonzero( np.logical_and(yNotNaN, np.logical_and(yNotBlown, yNotMissing)) )[0]    

    errs = {}
    
    for m in measures:
        if zPredTest is not None:
            if len(zPredTest.shape) == 2 and m not in ['AUC', 'ACC', 'ACCD1', 'CM'] and (m != 'PoissonLL' or ZType == 'count_process'):
                ...
                if m == 'CM':
                    if zOk.size==0:
                        errs[m] = np.ones((zPredTest.shape[2], zPredTest.shape[2], zPredTest.shape[1]))*np.nan
                    errs['mean'+m] = np.mean(errs[m], axis=2)
                else:
                    errs['mean'+m] = np.mean(errs[m])

        if yPredTest is not None:
            if len(yPredTest.shape) == 2 and m not in ['AUC', 'ACC', 'ACCD1', 'CM'] and (m != 'PoissonLL' or YType == 'count_process'):
...
def computeLSSMIdError(sTrue, sId_in):
    ...
    if hasattr(sId_in, 'state_dim') and sTrue.state_dim == sId_in.state_dim:
        ...
        E = sId.makeSimilarTo(sTrue) #TEMP CM!!! This was needed to be commented temporarily to test something for avoiding error
    ...
    if hasattr(sId_in, 'zDims') and len(sId_in.zDims) > 0:
        try:
            ...
            if len(idZEigs) < len(trueZEigs):
                idZEigs = np.hstack( (idZEigs, np.zeros((len(trueZEigs)-len(idZEigs))) ) )
            if len(trueZEigs) < 9:
...
def evalSysId(sId, YTest=None, ZTest=None, YTrain=None, ZTrain=None, UTest=None, UTrain=None, trueSys=None, YType=None, ZType=None, useXFilt=False, missing_marker=None, undo_scaling=False):
    ...
    if YTest is not None:
        ...
        for saInd, step_ahead in enumerate(steps_ahead):
            perfDThis = evaluateDecoding(ZTest=ZTest, zPredTest=zPredTest[saInd], 
                                         YTest=YTest, yPredTest=yPredTest[saInd], 
                                YType=YType, ZType=ZType, missing_marker=missing_marker)
