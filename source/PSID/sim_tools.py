...
def generateRandomLinearModel(sysSettings):
    ...
    s.changeParams(
        {
            ...
            "Rz": Rz,
            "zDims": zDims
            + 1,  # plus 1 to be consistent with Matlab indices for zDims in simulated systems
        }
    )
    ...
    if "yZNZRLR" in sysSettings and sysSettings["yZNZRLR"] is not None:
        ...
        sc = 10 ** (np.random.rand() * np.diff(rngL) + rngL[0])
        zDimInds = np.array(
            [ind for ind in np.arange(s.state_dim) if ind in (s.zDims - 1)]
        )
        nonZDims = np.array(
            [ind for ind in np.arange(s.state_dim) if ind not in (s.zDims - 1)]
        )
        Cy = np.copy(s.C)
        CyXCovZDims = (
            Cy[:, zDimInds] @ s.XCov[np.ix_(zDimInds, zDimInds)] @ Cy[:, zDimInds].T
        )
        CyXCovNonZDims = (
            Cy[:, nonZDims] @ s.XCov[np.ix_(nonZDims, nonZDims)] @ Cy[:, nonZDims].T
        )
